...
void diagonal(FILE* f1,FILE* f2){
  ...
  PPM image;
   ...
   int mid1 = image.height / 2; // precisamos de pensar da seguinte maneira: dividir a imagem ao meio, tendo como referencial a altura, de cima até ao meio

    for (int j = 0; j < image.width; j++){
     for (int k = 0; k < mid1; k++){ // precorre até ao meio de cima para baixo
     a = image.mat[k][j];
     image.mat[k][j] = image.mat[image.height-k-1][j];
     image.mat[image.height-k-1][j] = a; // trocam-se os pixeis, trocando os índices
     }
    }
        //agora imprimimos
    for (int j =0; j<image.height; j++){
